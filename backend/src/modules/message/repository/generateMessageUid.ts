...
export default async () => {
	try {
		return await generateMessageUid("MSG");
	} catch (err) {
		console.error(err);
...
const generateMessageUid: any = async (prefix: String) => {
	const randomString =
		prefix + "-" + String.fromCharCode(...randomNumberGenerator(5, 32));

	const uidExists = await MessageModel.findOne({
		uid: randomString,
	});

	if (uidExists) {
		console.error("Matched!", randomString);
		return await generateMessageUid(prefix);
	}
