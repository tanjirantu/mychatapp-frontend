...
const SharedFile = () => {
    const { activeMessagehead, attachments } = useAppSelector((state) => state.messages);
    ...
    const { isLoading, lazyFetch } = useQuery(getAllMessagesByRoomUidQuery, {
        skip:
            ...
            attachments.results[getUrlParams({ uid: receiverUid, includeOnly: fileTypes })] !== undefined
                ? true
                : false,
        onQueryEnd: (state) => {
            dispatch(
    ...
    const results =
        attachments.results[getUrlParams({ uid: activeMessagehead?.friends[0].uid, includeOnly: fileTypes })];
